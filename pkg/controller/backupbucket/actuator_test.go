	...
	Describe("#Reconcile", func() {
		Context("client creation fails during reconiliation", func() {
			It("should error", func() {
				...
				azureClientFactory.EXPECT().Group().Return(azureGroupClient, fmt.Errorf("resource group client creation failed"))
		...
		Context("when the backupBucket configured without immutability does not exist", func() {
			...
			It("should error if bucket creation fails", func() {
				// create the bucket
				azureBlobContainersClient.EXPECT().CreateContainer(ctx, resourceGroupName, storageAccountName, backupBucket.Name).Return(armstorage.BlobContainersClientCreateResponse{}, fmt.Errorf("blob storage container creation failed"))
		...
		Context("when the backupBucket configured with unlocked immutability does not exist", func() {
			...
			It("should error if adding the immutability policy fails", func() {
				...
				azureBlobContainersClient.EXPECT().CreateOrUpdateImmutabilityPolicy(ctx, resourceGroupName, storageAccountName, backupBucket.Name, &immutabilityDays).Return(fmt.Errorf("adding the immutability policy failed"))
		...
		Context("when the backupBucket configured with locked immutability does not exist", func() {
			...
			It("should error if locking the immutability policy fails after creation of bucket and addition of policy", func() {
				...
				azureBlobContainersClient.EXPECT().LockImmutabilityPolicy(ctx, resourceGroupName, storageAccountName, backupBucket.Name, &etag2).Return(fmt.Errorf("locking the immutability policy failed"))
		...
		Context("when a backupBucket configured for locked immutability already exists without immutability", func() {
			...
			It("should error if adding the lock fails", func() {
				...
				etag2 := "backupbucket-first-etag-2"
				azureBlobContainersClient.EXPECT().GetImmutabilityPolicy(ctx, resourceGroupName, storageAccountName, backupBucket.Name).Return(nil, true, &etag2, nil)
				azureBlobContainersClient.EXPECT().LockImmutabilityPolicy(ctx, resourceGroupName, storageAccountName, backupBucket.Name, &etag2).Return(fmt.Errorf("locking the policy failed"))
...
func mockGeneratedSecretCreation(
	...
	objectKey := client.ObjectKeyFromObject(generatedSecret)
	c.EXPECT().Get(ctx, objectKey, generatedSecret.DeepCopy()).Return(apierrors.NewNotFound(schema.GroupResource{}, generatedSecret.Name))
